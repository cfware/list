import ShadowElement, {html, template, css, adoptedStyleSheets, booleanAttribute, addCleanups} from '@cfware/shadow-element';
...
export default class CFWareList extends ShadowElement {
    ...
    _onheadersize() {
        ...
        for (const ele of [heads, bodys].flat()) {
            ele.width = '';
        }
        ...
        const columns = [{className: 'minimum'}, ...this[listColumns]]
            .map(({className}, index) => [className === 'minimum', index])
            .sort(([a], [b]) => a === b ? 0 : (a ? -1 : 1))
            .map(([, index]) => [
                heads[index],
                bodys[index],
                index === heads.length - 1 ? scrollWidth : 0
            ]);
